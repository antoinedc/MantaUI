...
- MANTA HAS CI (GitHub Actions on `manta-dev-runner`): one required check, `typecheck-test` (`.github/workflows/required-checks.json`), enforced by the `main` branch ruleset. It covers typecheck, tests, the gitleaks secret scan and a dependency audit that runs only when the PR changes the dependency set. There is NO `/merge` command workflow (removed with BET-247); the PM merges auto-tier PRs with `gh pr merge --merge` directly once required checks are green. `npm run typecheck && npm test` locally is the fallback when CI is queued.
