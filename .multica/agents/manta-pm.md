...
**`main` is governed by ONE system: the repository RULESET** (Settings → Rules →
Rulesets → "main"). The legacy per-branch protection rule was deleted
2026-07-27, because GitHub enforces the UNION of both and two overlapping
configs meant an edit in one screen silently did nothing. **Never re-add a
classic branch protection rule.** The ruleset is deliberately NOT strict
(branches need not be up to date before merging) — do not turn that on: with
several agent PRs in flight it forces every merge to invalidate every other PR.
If you change a required check's name, update the ruleset FIRST, then merge the
workflow change — requiring a context no job produces blocks every PR forever.
