...
- **On merge to main: CI builds the bundle and publishes it as a release
  ...
  manifest (git sha + filename + sha256), scp'd over the same self-hosted runner +
  deploy key as `website-deploy.yml`. **No auto-PR, no commit to main, no
...
Everything now lives in `.github/workflows/ci.yml` as **three** jobs:
...
| `E2E Smoke Test` | no | Electron + Xvfb smoke; flakier than the deterministic gates, so a judgment call |
