...
export function createPairingRegistry({ ttlMs = PAIRING_TTL_MS, now = () => Date.now() } = {}) {
  ...
  function issue() {
    active = { code: genPairingCode(), expiresAt: now() + ttlMs };
  ...
  // distinguish "wrong code" from "expired" — like `claim`, a guesser learns
  // only "no".
  function check(code) {
    if (!isValidPairingCode(code)) return false;
    if (!active) return false;
  ...
  return { issue, consume, check, clear, hasActive };
}
...
export function createAuthEngine({
  ...
  // issue. Returns the code + box_id so the caller can render it / a QR.
  function pair() {
  ...
  // "no", never partial progress). A stray `verify` field in the body is
  // intentionally ignored (not read), so a not-yet-updated old client that
  // still sends it keeps working.
  //
  // TWO PATHS, keyed by device identity rather than a verify code:
  //   • a claim carrying `device_id` OR `name` → joiner path: PROVISION/RESUME
  //     a DISTINCT device ("resume" when a matching `device_id` is given,
  //     "provision" — a freshly minted id — when only `name` is). Never the
  //     primary, so a joiner can never impersonate the desktop or an existing
  //     device.
  //   • a bare claim (neither `device_id` nor `name`) → legacy first-pair path:
  //     resume the PRIMARY (shared box_token) device and return
  //     `box_token = auth.box_token` exactly as before — existing paired
