...
const TOOL_COUNT = 64; // 16 reads (BET-1164 + BET-1383 read_rollups/read_ledger) + read_inbox (BET-1397) + 3 watcher tools (BET-1165) + 3 drill-down read verbs (BET-1399) + 3 passive context verbs (P1c) + 15 §7 projects/sessions control tools (P4) + 17 §7 work record+dispatch control tools (incl. P6 work_schedule read + work_handoff) + 6 §11 completion stages: review/merge/release/verify/complete/rollback
...
test("registry exposes every cto read tool with a complete shape, all mode auto", () => {
  ...
  assert.deepEqual(
    ...
    [
      ...
      // verified-completion transition this PR legitimately owns.
      "work_review",
      "work_merge",
      "work_release",
      "work_verify",
