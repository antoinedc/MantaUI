...
describe("prefillFromPairLink (BET-335 deep-link prefill)", () => {
  ...
  // BET-700: a deep-link that carries a stray `&verify=` (old desktop QRs / CLI
  // pair links) is IGNORED — it still prefills the plain { boxId, code } payload
  // whether the value is well-formed or malformed.
  it("ignores a stray &verify= param on a pair link (BET-700)", () => {
    expect(prefillFromPairLink(`${validLink}&verify=K7Q2`)).toEqual({
      boxId: VALID_BOX,
      code: "123456",
    });
    expect(prefillFromPairLink(`${validLink}&verify=K7`)).toEqual({
      boxId: VALID_BOX,
      code: "123456",
    });
    expect(prefillFromPairLink(`${validLink}&verify=K7Q2X`)).toEqual({
      boxId: VALID_BOX,
      code: "123456",
    });
  });
