...
const SKIP_REASON: Record<string, string> = {
  ...
  SettingsRow:
    "single-surface primitive: 1 adopting file (Settings.tsx, the settings form's schema rows — ToggleField, SegmentedField, and the plugins/launcher checkbox rows). The settings panel is a single surface; owner-approved exemption from the two-adopter rule (BET-623), like Toggle.",
  MessageBubble:
...
describe("M527 primitive rules", () => {
  ...
  describe("D4 — chrome-ownership allowlist (BET-588)", () => {
    ...
    const CHROME_OWNERS: Record<string, string[]> = {
      ...
      "bg-inset": ["MediaBody.tsx", "OutputWell.tsx", "ArtifactsPanel.tsx", "ArtifactPreview.tsx", "PaletteShell.tsx", "DeviceFlow.tsx", "CloneFromGitHub.tsx", "ConnectPanel.tsx", "inlineEmbed.tsx"], // the recessed output well — `.tool-b`/`.ask-cmd` (BET-636); plus the Artifacts panel's segmented tab track, which uses the same recessed-inset surface per its conformance mockup (`.mk-tabs`, BET-659); the preview overlay's image body — the recessed viewport mandated by the same mockup (`.pvbody` → `--inset`, BET-661); the PaletteShell footer key-legend strip (BET-695); the device-code box (`--inset`) shared by DeviceFlow/CloneFromGitHub per the §7.4 mockup `.dcode` (BET-796); the Connect panel's BELOW-panel install log pane, which uses the recessed-inset surface per the BET-1007 design contract (`--inset`); the MediaBody reserved-aspect box (BET-1148), the media card's recessed viewport; the shared inline-embed (media + widget) reserved box and degraded placeholder, extracted from MediaBody into inlineEmbed.tsx (BET-1325)
      ...
      "text-[12.5px]": ["ToolCard.tsx", "OutputWell.tsx", "Button.tsx", "SettingsRow.tsx", "CloneFromGitHub.tsx"], // the 12.5px mono chrome — ToolCard header + OutputWell well (BET-636), plus the pre-existing Button label and SettingsRow help which already owned it before this primitive tracked it; and the clone-flow "into <path>" line per the §7.4 mockup `.bar`-adjacent copy (BET-796)
    };
