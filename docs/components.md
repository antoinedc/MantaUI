...
| Chip | `src/renderer/Chip.tsx` | 2 — `NewSessionScreen.tsx` (folder chip, and the branch side of the worktree SplitChip); `ModelPicker.tsx` (via the shared module) | `on` (the accent "active" state); `onClick`; `title`; `children`; `hook`. No `size` prop — one size only (29px hit area) |
| SplitChip | `src/renderer/Chip.tsx` | 2 — `ModelPicker.tsx` (model ▸ effort split); `NewSessionScreen.tsx` (split branch/worktree chip) | `left`/`right` (ReactNode); `onLeftClick`/`onRightClick`; `rightAccent` (accent-tx + semibold on the right — replaces the composer's old inline `var(--accent-tx)`); `leftTitle`/`rightTitle`; `hook`. Both co-reside in `Chip.tsx` because they share the shell and must not diverge |
