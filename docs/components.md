...
| ModelChecklist | `src/renderer/ModelChecklist.tsx` | 2 — `CustomProviderForm.tsx` (probe results), `AccountsCard.tsx` (saved endpoint) | `onBulkChange` PRESENT → filter box + All/None row (batch); ABSENT → neither. No second `searchable`/`mode` prop — the two always co-vary. All/None act only on the VISIBLE (filtered) ids. Rows via `ListRow` + `Checkbox` (`ariaLabel={id}`) |
